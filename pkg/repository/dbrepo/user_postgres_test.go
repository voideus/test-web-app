...
func TestPostgresDBRepoAllUsers(t *testing.T) {
	...
	if err != nil {
		t.Errorf("all users reported an error: %s", err)
	}

	if len(users) != 1 {
		t.Errorf("All users reported wrong size; expected 1 but got %d", len(users))
	}
	...
	if len(users) != 2 {
		t.Errorf("All users reported wrong size after insert; expected 2 but got %d", len(users))
	}
}
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if user.ID != 1 {
		t.Errorf("Get user returned user with wrong id; expected 1, but got %d", user.ID)

	}

	_, err = testRepo.GetUser(3)
	if err == nil {
		t.Error("No error returned when getting user with non existent id")
	}
}
...
func TestPostgresDBRepoGetUserByEmail(t *testing.T) {
	...
	if user.Email != "admin@example.com" {
		t.Errorf("Get user returned user with wrong email; expected admin@example.com, but got %d", user.ID)
...
func TestPostgresDBRepoUpdateUser(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error updating user with id %d, err: %s", 2, err)
	}
...
func TestPostgresDBRepoDeleteUser(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Error deleting user with id 2, err: %s", err)
	}

	_, err = testRepo.GetUser(2)
	if err == nil {
		t.Error("retrieved user with id 2 which should be deleted")
	}
}
...
func TestPostgresDBRepoResetPassword(t *testing.T) {
	...
	if err != nil {
		t.Error("error checking user's password: ", err)
	}

	if !matches {
		t.Errorf("Password should match 'password' but didn't.")
	}
}
