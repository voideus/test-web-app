...
func Test_app_enableCORS(t *testing.T) {
	...
	for _, e := range tests {
		...
		req := httptest.NewRequest(e.method, "http://testing", nil) // target "http://testing" can be any temp url
...
func Test_app_authRequired(t *testing.T) {
	...
	for _, e := range tests {
		...
		if e.expectAuthorized && rr.Code == http.StatusUnauthorized {
			t.Errorf("%s: got %d, and should not have", e.name, http.StatusUnauthorized)
		}

		if !e.expectAuthorized && rr.Code != http.StatusUnauthorized {
			t.Errorf("%s: did not got %d, and should have", e.name, http.StatusUnauthorized)
		}
	}
}
